...
export default class SwarmImageReader {
  ...
  get srcset(): string | undefined {
    ...
    return resposiveSizes.reduce(
      (srcset, size) =>
        `${srcset ? srcset + ',' : ''} ${size} ${responsiveSources[size]}`,
      ''
    );
  }
  ...
  getOptimizedSrc(size?: number): string {
    ...
    const sizes = Object.keys(this.responsiveSources)
    ...
    const optimizedSrc = optimized
      ? this.responsiveSources[optimized + 'w']
      : this.responsiveSources[largest + 'w'];
