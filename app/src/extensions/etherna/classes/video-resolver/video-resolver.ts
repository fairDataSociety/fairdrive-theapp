...
export default class VideoResolver {
  ...
  private async fetchVideo(path: string) {
    try {
      const { data: video } = await this.axios.get<IndexVideo>(`videos/fdrive/${path}`)
      return video
    } catch (error) {
      return null
    }
  ...
  private async fetchComments(path: string) {
    try {
      const { data: comments } = await this.axios.get<IndexVideoComment[]>(`videos/fdrive/${path}/comments`)
      return comments
    } catch (error) {
      return []
    }
  ...
  private async fetchVideoMeta(reference: string) {
    try {
      const { data: meta } = await this.axios.get<SwarmVideoRaw>(`files/${reference}`, {
